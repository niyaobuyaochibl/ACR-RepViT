...
class BlockWithSA(nn.Module):
    ...
    def forward(self, x):
        ...
        sa_out = mixer_out * self.sa(mixer_out)
        x = x + self.drop_path(self.layer_scale_1.unsqueeze(-1).unsqueeze(-1) * sa_out)
        x = x + self.drop_path(self.layer_scale_2.unsqueeze(-1).unsqueeze(-1) * self.mlp(self.norm2(x)))
        return x

class ACRRepViTSA(nn.Module):
    def __init__(
